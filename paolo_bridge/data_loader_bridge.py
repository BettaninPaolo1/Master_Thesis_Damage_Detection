...
SEQ_LEN = 600
N_CLASS = 6
ACCELERATIONS = 0 # 0 per U_concat_, 1 per U2_concat_ [cite: 6]
...
def load_and_process_data(path_data, is_training=True, added_snr=100):
    """
    Carica i segnali seguendo la logica di slicing del file VAE_for_MCMC.
    """
    print(f"--- Caricamento Dati: {os.path.basename(path_data)} ---")
    
    # A. Caricamento Labels e Filtraggio Classe 0 [cite: 4]
    try:
    ...
    # Filtra solo i casi non sani (Classe 1-6) [cite: 4]
    ...
    if is_training:
        ...
        # Carica le medie del training se siamo in fase di test [cite: 5]
    ...
    # D. Lettura e Processing Canale per Canale [cite: 6, 7]
    for i_ch, ch_num in enumerate(CHANNELS):
        ...
        for i_idx, original_idx in enumerate(valid_indices):
            ...
            # Corruzione con rumore Gaussiano [cite: 7, 8]
            rms_val = RMS(signal)
        ...
        if is_training:
            # Calcolo statistiche per la normalizzazione [cite: 8]
            signals_means[i_ch] = np.mean(X_noise[:, :, i_ch])
            signals_stds[i_ch] = np.std(X_noise[:, :, i_ch])

    # E. Salvataggio Statistiche e Normalizzazione Z-Score [cite: 9]
    if is_training:
    ...
    # F. Creazione Vettori Label (Livello - 0.25) [cite: 10, 11]
    y_vector = np.zeros((n_ist, N_CLASS))
    for i_idx, original_idx in enumerate(valid_indices):
        cls_idx = labels_class[original_idx] - 1 # Mappa 1-6 su 0-5 [cite: 11]
        lvl = labels_level[original_idx]
        if 0 <= cls_idx < N_CLASS:
            y_vector[i_idx, cls_idx] = lvl - 0.25 # [cite: 11]
...
if __name__ == "__main__":
    ...
    X, y = load_and_process_data(DATA_DIR_TRAIN, is_training=True, added_snr=100)
    ...
    if X is not None:
        ...
        plt.title(f"Verifica Segnale - Indice {idx} (Classe: {np.argmax(y[idx])+1})")
        ...
        print(f"\nVisualizzazione del campione {idx} completata.")
        plt.show()
    else:
        print("Impossibile generare il plot: dati non caricati.")