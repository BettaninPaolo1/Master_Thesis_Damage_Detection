...
RESULTS_DIR = os.path.join(SCRIPT_DIR, "results_advi_bridge")
...
def print_uncertainty_report(trace, y_true):
    ...
    print("\n" + "="*55)
    print(f"   REPORT INCERTEZZA ADVI BRIDGE (Scala Likelihood: {LIKELIHOOD_SCALE})")
    print("="*55)
...
if __name__ == "__main__":
    ...
    if isinstance(y_scale_factor_loaded, np.ndarray):
        y_scale_val = float(y_scale_factor_loaded.item()) if y_scale_factor_loaded.ndim == 0 else float(y_scale_factor_loaded[0])
    else:
        y_scale_val = float(y_scale_factor_loaded)
        
    print(f"Parametri caricati: Z_mean {z_mean.shape}, Y_scale {y_scale_val:.4f}")
    ...
    try:
        ...
        print("Classificatore CVAE Bridge caricato.")
    except:
    ...
    #IDX = 2764 # Fisso per debug

    print(f"\n--- Analisi ADVI Bridge su Campione {IDX} ---")
    ...
    logp_op = RealNVPLLogpOp(realnvp, z_sample_norm, y_scale_val)
    ...
    print(f"[End] ADVI Bridge completato in {elapsed:.2f} secondi.")
    ...
    # 1. Istogrammi (Layout 2x3 per 6 classi)
    ...
    plt.suptitle(f'ADVI Bridge Posterior - Sample {IDX}\nTime: {elapsed:.2f}s', fontsize=16)
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, f"advi_bridge_histograms_{IDX}.png"))
    plt.show()

    # 2. ELBO Plot
    plt.figure(figsize=(8, 5))
    plt.plot(approx.hist)
    plt.title("ADVI Bridge Loss (ELBO) Convergence")
    plt.xlabel("Iterations")
    plt.ylabel("Loss")
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(RESULTS_DIR, f"advi_bridge_elbo_{IDX}.png"))