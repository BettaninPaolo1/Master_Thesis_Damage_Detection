...
# ==========================================
# 1. CONFIGURAZIONE (BRIDGE - DEEP FiLM 3L)
# ==========================================
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))

# Cartelle
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "models_evolution") 
RESULTS_DIR = os.path.join(SCRIPT_DIR, "results_svgd_bridge_stats4")
if not os.path.exists(RESULTS_DIR):
    os.makedirs(RESULTS_DIR)

# Percorsi Modelli (AGGIORNATI AL NUOVO TRAINING)
ENCODER_PATH = os.path.join(OUTPUT_DIR, "encoder_bridge.keras")
CLASSIFIER_PATH = os.path.join(OUTPUT_DIR, "classifier_bridge.keras")
WEIGHTS_PATH = os.path.join(OUTPUT_DIR, "realnvp_bridge_3layer_film_weights.weights.h5") # <--- AGGIORNATO
...
# Parametri Architettura (DEVONO MATCHARE IL NUOVO TRAINING)
LATENT_DIM = 4          
NUM_CLASSES = N_CLASS   # 6 per il bridge
NUM_COUPLING_LAYERS = 14 # <--- AGGIORNATO (era 4)
HIDDEN_DIM = 128         # <--- AGGIORNATO (era 64)
ACTIVATION = "relu"
...
# ==========================================
# 2. DEFINIZIONE MODELLI (Deep FiLM 3L + Residuals)
# ==========================================
...
class RealNVP_Bridge_3L(keras.Model):
    ...
    def call(self, x, y): # Tolto training=False perché non abbiamo dropout in questa versione
        log_det_inv = 0
        z = x
    ...
    def log_prob_grads(self, x_fixed, particles, y_scale_factor_tf):
        ...
        with tf.GradientTape() as tape:
            ...
            # Forward pass
            z_pred, log_det_inv = self(x_batch, particles_scaled)
...
if __name__ == "__main__":
    ...
    try:
        keras.utils.get_custom_objects().update({
            ...
            "RealNVP_Bridge_3L": RealNVP_Bridge_3L
        })
    except: pass
    ...
    print("--- 2. Caricamento Modelli Bridge (Deep FiLM 3L) ---")
    ...
    # Inizializzazione Architettura AGGIORNATA
    realnvp = RealNVP_Bridge_3L(NUM_COUPLING_LAYERS, LATENT_DIM, NUM_CLASSES, HIDDEN_DIM)
