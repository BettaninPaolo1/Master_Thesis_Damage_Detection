...
# Nome file CSV
CSV_FILENAME = "results_svgd_batch.csv"
...
if __name__ == "__main__":
    ...
    # 3. Livelli Scalari (Aggiunto pred_levels_cvae basato sulla classe scelta dal CVAE)
    true_levels = y_true[idx_range, true_cls_idx]
    pred_levels_cvae = y_vae[idx_range, vae_cls_idx]  # <--- MANCAVA QUESTO
    
    pred_levels_mean_selected = y_mean[idx_range, mean_cls_idx]
    pred_levels_mode_selected = y_mode[idx_range, mode_cls_idx]
    
    # (Manteniamo questo solo se ti serve ancora nei vecchi plot (es. Plot 5))
    vae_levels = y_vae[idx_range, true_cls_idx] 

    # 4. Metriche Campione (MAE) e Incertezza
    ...
    # ------------------------------------------
    # --- 1. Accuratezza Classificazione ---
    acc_cvae = accuracy_score(true_cls, vae_cls) # Usa vae_cls definito sopra
    acc_mean = accuracy_score(true_cls, mean_cls)
    acc_mode = accuracy_score(true_cls, mode_cls)

    # --- 2. Metriche Vettoriali (su tutto l'array [0, 0, 0.4, 0...]) ---
    ...
    smape_vec_cvae = calculate_smape(y_true, y_vae)
    smape_vec_mean = calculate_smape(y_true, y_mean)
    smape_vec_mode = calculate_smape(y_true, y_mode)

    # --- 3. Metriche Scalari (Regressione PURA sull'intensità del danno) ---
    ...
    with open(SUMMARY_PATH, "w", encoding="utf-8") as f:
            ...
            # --- TABELLA A 3 COLONNE ---
            f.write("-" * 105 + "\n")
            f.write(f"{'METRICA':<20} | {'CVAE (Baseline)':<25} | {'SVGD MEAN Est':<25} | {'SVGD MODE Est':<25}\n")
            f.write("-" * 105 + "\n")
            ...
            if np.sum(mask_wrong) > 0:
                ...
                # Aggiunta terza colonna anche alla stampa dell'errore finale (che avevi inserito)
