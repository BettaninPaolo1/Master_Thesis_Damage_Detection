...
LAMBDA_SPARSITY = 10.0  # Valore moderato per pulire il rumore
...
if __name__ == "__main__":
    ...
    # ==========================================
    # TEST DIAGNOSTICO "RADAR" (Inserire prima del pm.Model)
    # ==========================================
    print("\n--- RADAR DIAGNOSTICO: Cosa vede il modello? ---")
    print(f"Cercando di identificare danno sulla classe vera (Vedi y_true sopra)...")
    
    # Prepara tensori fissi per il test
    x_test_tf = tf.cast(z_sample_norm, dtype=tf.float32)
    if len(x_test_tf.shape) == 1: x_test_tf = tf.expand_dims(x_test_tf, 0)
    
    scores = []
    
    # 1. Testiamo l'ipotesi "Sano" (Tutto zero)
    y_zero = tf.zeros((1, NUM_CLASSES), dtype=tf.float32)
    loss_zero, _ = realnvp.value_and_gradient(x_test_tf, y_zero, tf.constant(y_scale_factor, dtype=tf.float32))
    print(f" > Ipotesi SANO (0.0): Likelihood = {loss_zero.numpy():.2f}")
    
    # 2. Testiamo ogni classe singolarmente con danno medio (0.35)
    damage_val = 0.35
    for c in range(NUM_CLASSES):
        y_probe = np.zeros((1, NUM_CLASSES), dtype=np.float32)
        y_probe[0, c] = damage_val
        y_probe_tf = tf.constant(y_probe)
        
        # Chiediamo al modello il punteggio
        loss_val, _ = realnvp.value_and_gradient(x_test_tf, y_probe_tf, tf.constant(y_scale_factor, dtype=tf.float32))
        score = loss_val.numpy()
        scores.append(score)
        
        mark = " <--- VERO!" if y_true[c] > 0.1 else ""
        print(f" > Ipotesi CLASSE {c+1} (Danno {damage_val}): Likelihood = {score:.2f}{mark}")

    # Analisi rapida
    best_class = np.argmax(scores)
    print(f"--- RISULTATO RADAR: Il modello preferisce la CLASSE {best_class+1} ---")
    print("Se la classe preferita non è quella vera, ADVI non potrà mai trovarla.\n")
    # ==========================================
    ...
    with pm.Model() as model:
        ...
        # Spinge verso zero i valori inutili, aiutando il modello a decidere
        pm.Potential("sparsity", -LAMBDA_SPARSITY * pt.sum(pt.abs(label_vector)))
